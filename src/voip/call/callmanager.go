...
// FrameSize returns the active codec frame size in samples, or 0 if no codec is
// initialized yet. Used by headless audio sources to pace uplink playback.
func (m *CallManager) FrameSize() int {
	m.mu.Lock()
	defer m.mu.Unlock()
	if m.codec == nil {
		return 0
	}
	return m.codec.FrameSize()
}
